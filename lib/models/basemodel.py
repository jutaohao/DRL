...
class BaseModel():
    ...
    def save_weights(self, epoch:int, is_best:bool=False):
        ...
        weight_dir = os.path.join(
            self.opt.outf, self.opt.name, 'train',self.opt.normal_class, 'weights')
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            res = self.test()
            if res['ValidAUC'] > best_auc:
                best_auc = res['ValidAUC']
                test_best_auc = res['TestAUC']
            self.visualizer.print_current_performance(res, test_best_auc)
        # numpy.save('weight_%s_loss_%s.npy' %(self.opt.w_adv,self.opt.normal_class), self.loss_my)
